...
def action_modify_optional_header_1(input_file_,enable_log=False):
    with open(input_file_, "rb") as f1:
        bytez = f1.read()
    if enable_log:
        m1 = hashlib.sha256()
        m1.update(bytez)
        logging.info("action_modify_optional_header_1 before file:" + m1.hexdigest())
    ...
    temp_name = temp_name + "-action_modify_optional_header_1-" + file_name
    ...
    if enable_log:
        m2 = hashlib.sha256()
        m2.update(bytez1)
        logging.info("action_modify_optional_header_1 after file:" + m2.hexdigest())

if __name__=="__main__":
    init_environment()
    input_file =r"D:\毕业设计\example1\source_file\sample1.exe"
    action_modify_optional_header_1(input_file_=input_file,enable_log=True)